            ...
            <div id="{$this-article}-floats" class="back">
                <xsl:call-template name="main-title">
                    <xsl:with-param name="contents">
                        <span class="generated">Gleitobjekte</span>
                    </xsl:with-param>
                </xsl:call-template>
                <xsl:apply-templates/>
            </div>
...
<!--        </xsl:for-each>-->
    ...
    <xsl:template name="footer-metadata">
        ...
        <xsl:for-each select="front/article-meta | front-stub">
            <xsl:if test="article-categories | kwd-group | counts |
                ...
                <div class="metadata">
                    <h4 class="generated">
                        <xsl:text>Artikelinformationen (Fortsetzung)</xsl:text>
                    </h4>
                    <div class="metadata-group">
                        <xsl:apply-templates mode="metadata"
        ...
        <xsl:if test="normalize-space(string($process-warnings))">
            ...
            <div class="metadata">
                ...
                <div class="row">
                    <div class="col-12">
                        <h4 class="generated">
                            <xsl:text>Verarbeitungshinweise</xsl:text>
                        </h4>
                        <p>Folgende Warnungen wurden aufgrund problematischer Auszeichnung ausgegeben:</p>
                        <div class="metadata-group">
    ...
    <xsl:template match="journal-id[normalize-space()]" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Zeitschrift-ID</xsl:text>
    ...
    <xsl:template match="journal-id[not(normalize-space())]" mode="metadata"/>
    ...
    <xsl:template match="notes" mode="metadata">
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Hinweise</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <!-- journal-title and journal-subtitle are suppressed in the front metadata panel -->
    <xsl:template match="journal-title | journal-subtitle" mode="metadata"/>
    ...
    <xsl:template match="abbrev-journal-title" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Abgekürzter Titel</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="trans-title" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Übersetzter Titel</xsl:text>
    ...
    <xsl:template match="trans-subtitle" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Übersetzter Untertitel</xsl:text>
                <xsl:for-each select="(../@xml:lang|@xml:lang)[last()]">
                    <xsl:text> (</xsl:text>
                    <span class="data">
                        <xsl:value-of select="."/>
    ...
    <xsl:template match="ext-link" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Externer Link</xsl:text>
    ...
    <xsl:template match="self-uri" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Dokument-URI</xsl:text>
            </xsl:with-param>
            <xsl:with-param name="contents">
                <a href="{@xlink:href}">
                    <xsl:choose>
    ...
    <xsl:template match="product" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Produktinformation</xsl:text>
            </xsl:with-param>
            <xsl:with-param name="contents">
                <xsl:choose>
    ...
    <xsl:template match="copyright-statement" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Urheberrechtshinweis</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="history/date" mode="metadata">
        ...
        <xsl:if test="normalize-space(string($date-content))">
            <xsl:call-template name="metadata-labeled-entry">
                <xsl:with-param name="label">
                    <xsl:choose>
                        <xsl:when test="@date-type='accepted'">Annahmedatum</xsl:when>
                        <xsl:when test="@date-type='received'">Eingangsdatum</xsl:when>
                        <xsl:when test="@date-type='rev-request'">Datum der Überarbeitungsanforderung</xsl:when>
                        <xsl:when test="@date-type='rev-recd'">Datum der überarbeiteten Fassung</xsl:when>
                        <xsl:when test="@date-type='initial-publication'">Erstveröffentlichung</xsl:when>
                        <xsl:when test="@date-type='correction'">Korrektur</xsl:when>
                        <xsl:when test="@date-type='latest-version'">Neueste Fassung</xsl:when>
                        <xsl:otherwise>
        ...
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:choose>
                    ...
                    <xsl:otherwise>
                        <xsl:text>Veröffentlichungsdatum</xsl:text>
                        <xsl:call-template name="append-pub-type"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:with-param>
        ...
        <xsl:if test="volume | volume-id | volume-series">
            <xsl:choose>
                <xsl:when test="not(volume-id[2]) or not(volume)">
                    ...
                    <xsl:call-template name="metadata-labeled-entry">
                        <xsl:with-param name="label">Band</xsl:with-param>
                        <xsl:with-param name="contents">
    ...
    <xsl:template match="volume-series" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Reihe</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
      ...
      issue-sponsor | issue-part"/>
        <xsl:choose>
            <xsl:when
                ...
                <xsl:call-template name="metadata-labeled-entry">
                    <xsl:with-param name="label">Ausgabe</xsl:with-param>
                    <xsl:with-param name="contents">
    ...
    <xsl:template match="issue-title" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Ausgabetitel</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="issue-sponsor" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Ausgaben-Sponsor</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="issue-part" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Ausgabenteil</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="elocation-id" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Elektronischer Standortbezeichner</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="supplement" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Supplementinformation</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="related-article | related-object" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Verwandter </xsl:text>
                <xsl:choose>
                    <xsl:when test="self::related-object">Gegenstand</xsl:when>
    ...
    <xsl:template match="conference" mode="metadata">
        ...
        <xsl:choose>
            <xsl:when
                ...
                <xsl:call-template name="metadata-labeled-entry">
                    <xsl:with-param name="label">Konferenz</xsl:with-param>
                    <xsl:with-param name="contents">
    ...
    <xsl:template match="conf-date" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Konferenzdatum</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="conf-num" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Konferenznummer</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="conf-loc" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Konferenzort</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="conf-sponsor" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Konferenzsponsor</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="conf-theme" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Konferenzthema</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="article-id" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:choose>
                    <xsl:when test="@pub-id-type='art-access-id'">Zugangs-ID</xsl:when>
                    <xsl:when test="@pub-id-type='coden'">Coden</xsl:when>
                    <xsl:when test="@pub-id-type='doi'">DOI</xsl:when>
                    <xsl:when test="@pub-id-type='manuscript'">Manuskript-ID</xsl:when>
                    <xsl:when test="@pub-id-type='medline'">Medline-ID</xsl:when>
                    <xsl:when test="@pub-id-type='pii'">Verlags-ID</xsl:when>
                    <xsl:when test="@pub-id-type='pmid'">PubMed-ID</xsl:when>
                    <xsl:when test="@pub-id-type='publisher-id'">Dokumentnummer</xsl:when>
                    <xsl:when test="@pub-id-type='sici'">Zeitschriftenbeitrags-ID</xsl:when>
                    <xsl:when test="@pub-id-type='doaj'">DOAJ-ID</xsl:when>
                    <xsl:when test="@pub-id-type='arXiv'">arXiv.org-ID</xsl:when>
                    <xsl:otherwise>
                        <xsl:text>Artikel-ID</xsl:text>
                        <xsl:for-each select="@pub-id-type">
                            <xsl:text> (</xsl:text>
                            <span class="data">
                                <xsl:value-of select="."/>
    ...
    <xsl:template match="contract-num" mode="metadata">
        <!-- only in 2.3 -->
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Vertrag</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="contract-sponsor" mode="metadata">
        <!-- only in 2.3 -->
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Vertragssponsor</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="grant-num" mode="metadata">
        <!-- only in 2.3 -->
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Fördernummer</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="grant-sponsor" mode="metadata">
        <!-- only in 2.3 -->
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Förderungsgeber</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="funding-source" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Gefördert durch</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="award-id" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Förder-ID</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="principal-award-recipient" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Förderempfänger</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="principal-investigator" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Hauptforscher</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="funding-statement" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Förderung</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="title-group/alt-title" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:text>Alternativer Titel</xsl:text>
    ...
    <xsl:template name="contrib-identify">
        ...
        collab | collab-alternatives/* | name | name-alternatives/*">
            ...
            <xsl:if test="@equal-contrib='yes'">
                <xsl:call-template name="metadata-entry">
                    <xsl:with-param name="contents">
                        <span class="generated">(Gleichberechtigter Beitrag)</span>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
        </div>
    ...
    <xsl:template match="author-comment" mode="metadata">
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Kommentar</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="bio" mode="metadata">
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Biografie</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="on-behalf-of" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Im Auftrag von</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="author-notes" mode="metadata">
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Autorenhinweise</xsl:with-param>
            <xsl:with-param name="contents">
                <xsl:call-template name="named-anchor"/>
                <xsl:apply-templates mode="metadata"/>
            </xsl:with-param>
    ...
    <xsl:template match="author-notes/corresp" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">
                <xsl:call-template name="named-anchor"/>
                <xsl:text>Korrespondenz an</xsl:text>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="supplementary-material" mode="metadata">
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Ergänzungsmaterial</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="subj-group/subject" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Fachgebiet</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="series-title" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Reihentitel</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="series-text" mode="metadata">
        <xsl:call-template name="metadata-labeled-entry">
            <xsl:with-param name="label">Reihenbeschreibung</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="counts" mode="metadata">
        ...
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Anzahl</xsl:with-param>
            <xsl:with-param name="contents">
                <xsl:apply-templates mode="metadata"/>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="fig-count"
                  mode="metadata-label">Abbildungen
    </xsl:template>


    <xsl:template match="table-count"
                  mode="metadata-label">Tabellen
    </xsl:template>


    <xsl:template match="equation-count"
                  mode="metadata-label">Gleichungen
    </xsl:template>


    <xsl:template match="ref-count"
                  mode="metadata-label">Referenzen
    </xsl:template>


    <xsl:template match="page-count"
                  mode="metadata-label">Seiten
    </xsl:template>


    <xsl:template match="word-count"
                  mode="metadata-label">Wörter
    </xsl:template>
    ...
    <xsl:template mode="metadata"
        ...
        <xsl:call-template name="metadata-area">
            <xsl:with-param name="label">Benutzerdefinierte Metadaten</xsl:with-param>
            <xsl:with-param name="contents">
                <xsl:apply-templates mode="metadata"/>
            </xsl:with-param>
        </xsl:call-template>
    ...
    <xsl:template match="boxed-text | chem-struct-wrap | fig |
        ...
        <div class="{$gi} panel">
            ...
            <xsl:if test="not(self::table-wrap)">
                <xsl:apply-templates/>
                <xsl:apply-templates mode="footnote"
                                     select="self::table-wrap//fn[not(ancestor::table-wrap-foot)]"/>
            </xsl:if>
        ...
        <div class="glossary">
            ...
            <xsl:if test="not(normalize-space(string(title)))">
                <xsl:call-template name="block-title">
                    <xsl:with-param name="contents">
                        <span class="generated">Glossar</span>
                    </xsl:with-param>
                </xsl:call-template>
            </xsl:if>
    ...
    <xsl:template match="textual-form">
        <p class="textual-form">
            <span class="generated">[Textuelle Form]</span>
            <xsl:apply-templates/>
        </p>
    </xsl:template>
    ...
    <xsl:template match="table">
        <xsl:copy>
            ...
            <xsl:attribute name="class">
                <xsl:choose>
                    <xsl:when test="@class">table table-bordered
                        <xsl:value-of select="@class"/>
                    </xsl:when>
                    <xsl:otherwise>table table-bordered</xsl:otherwise>
                </xsl:choose>
            </xsl:attribute>
            <xsl:call-template name="named-anchor"/>
            <xsl:apply-templates/>
    ...
    <xsl:template match="private-char">
        <span>
            ...
            <span class="generated">[Sonderzeichen</span>
    ...
    <xsl:template match="glyph-data | glyph-ref">
        <span class="generated">(Zeichen nicht darstellbar)</span>
    </xsl:template>


    <xsl:template match="related-article">
        <span class="generated">[Verwandter Artikel:]</span>
        <xsl:apply-templates/>
    </xsl:template>


    <xsl:template match="related-object">
        <span class="generated">[Verwandter Gegenstand:]</span>
        <xsl:apply-templates/>
    </xsl:template>
    ...
    <xsl:template name="footnotes">
        <xsl:call-template name="backmatter-section">
            <xsl:with-param name="generated-title">Anmerkungen</xsl:with-param>
            <xsl:with-param name="contents">
    ...
    <xsl:template match="ack">
        <xsl:call-template name="backmatter-section">
            <xsl:with-param name="generated-title">Danksagungen</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="back/bio">
        <xsl:call-template name="backmatter-section">
            <xsl:with-param name="generated-title">Biografie</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="back/glossary | back/gloss-group">
        <xsl:call-template name="backmatter-section">
            <xsl:with-param name="generated-title">Glossar</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="back/ref-list">
        <xsl:call-template name="backmatter-section">
            <xsl:with-param name="generated-title">Literaturverzeichnis</xsl:with-param>
            <xsl:with-param name="contents">
    ...
    <xsl:template match="chem-struct-wrap | chem-struct-wrapper" mode="label-text">
        ...
        <xsl:call-template name="make-label-text">
            ...
            <xsl:with-param name="warning" select="$warning"/>
            <xsl:with-param name="auto-text">
                <xsl:text>Formel (chemisch) </xsl:text>
                <xsl:number level="any"/>
            </xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template match="supplementary-material" mode="label-text">
        ...
        <xsl:call-template name="make-label-text">
            ...
            <xsl:with-param name="warning" select="$warning"/>
            <xsl:with-param name="auto-text">
                <xsl:text>Ergänzungsmaterial </xsl:text>
    ...
    <xsl:template name="metadata-labeled-entry">
        ...
        <xsl:call-template name="metadata-entry">
            <xsl:with-param name="contents">
                <xsl:if test="normalize-space(string($label))">
                    <span class="metadata-label generated">
                        <strong>
                            <xsl:copy-of select="$label"/>
                            <xsl:text>: </xsl:text>
                        </strong>
    ...
    <xsl:template name="format-date">
        ...
        <xsl:for-each select="month|season">
            <xsl:apply-templates select="." mode="map"/>
            <xsl:text> </xsl:text>
        </xsl:for-each>
    ...
    <xsl:template match="month" mode="map">
        ...
        <xsl:choose>
            <xsl:when test="number() = 1">Januar</xsl:when>
            <xsl:when test="number() = 2">Februar</xsl:when>
            <xsl:when test="number() = 3">März</xsl:when>
            <xsl:when test="number() = 4">April</xsl:when>
            <xsl:when test="number() = 5">Mai</xsl:when>
            <xsl:when test="number() = 6">Juni</xsl:when>
            <xsl:when test="number() = 7">Juli</xsl:when>
            <xsl:when test="number() = 8">August</xsl:when>
            <xsl:when test="number() = 9">September</xsl:when>
            <xsl:when test="number() = 10">Oktober</xsl:when>
            <xsl:when test="number() = 11">November</xsl:when>
            <xsl:when test="number() = 12">Dezember</xsl:when>
            <xsl:otherwise>
                <xsl:apply-templates/>
            </xsl:otherwise>
        </xsl:choose>
    ...
    <xsl:template name="footer-branding">
        ...
        <!--    <div class="branding">-->
        <!--      <p>-->
        <!--        <xsl:text>Diese Ansicht wurde aus NISO-JATS-XML generiert mit </xsl:text>-->
        <!--        <b>-->
        <!--          <xsl:value-of select="$transform"/>-->
        <!--        </b>-->
        <!--        <xsl:text>. Die XSLT-Engine ist </xsl:text>-->
